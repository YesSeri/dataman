...
use crate::controller::input::{InputMode, StateMachine};
use crate::controller::{self, input};
...
impl Database {
    ...
    pub(crate) fn copy(&mut self) -> AppResult<()> {
        let table_name = self.get_current_table_name()?;
        let header = self.get_current_header()?;
    ...
    pub(crate) fn delete_column(&mut self) -> AppResult<()> {
        let table_name = self.get_current_table_name()?;
        ...
        self.last_command = PreviousCommand::new(
            Command::DeleteColumn,
            Some(format!("Deleted column {column} from {table_name}")),
        );
        Ok(())
    }
    ...
    fn submit_message(&mut self) {
        if let Some(queued_command) = &mut self.queued_command {
            ...
            queued_command.inputs.push(self.input.clone());
        }
        self.reset_input();
    }
    ...
    fn get_external_data(&mut self) -> AppResult<String> {
        TUI::get_external_editor_input(&self.input)
    }
            ...
            let res = match if let Event::Key(key) = event::read()? {
                ...
                Ok(command) => {
                    ...
                    let result = match command {
                        ...
                        Command::NextTable => self.next_table(),
                        Command::PrevTable => self.prev_table(),
                    ...
                    match command {
                        Command::RenameTable => {
                            let old_table_name = self.get_current_table_name()?;
                            self.character_index = old_table_name.len();
                            self.input = old_table_name;
                        }
    ...
    pub fn get_headers_and_rows(&mut self, limit: u32) -> AppResult<DataTable> {
        ...
        let first_table = self.get_current_table_name()?;
        self.get(limit, 0, first_table)
    }
    ...
    pub fn regex_transform(&mut self, inputs: Vec<String>) -> AppResult<()> {
        ...
        if let Some(transformation) = inputs.get(1) {
            if contains_capture_pattern && !transformation.is_empty() {
                ...
                // )?
                // };
                self.regex_capture_group_transform(&pattern, &header, transformation)?;
                return Ok(());
            }
        }
        self.regex_no_capture_group_transform(&pattern, &header)?;
        Ok(())
    }
    ...
    pub(crate) fn edit_cell(&mut self, inputs: Vec<String>) -> AppResult<()> {
        ...
        let data = self.get_cell(id, &header)?;
        let edit_result = inputs[0].to_owned();

        self.update_cell(header.as_str(), id, &edit_result)?;
        Ok(())
    }
